<template>
  <div class="campaign-category-settings">
    <!-- 页面标题 -->
    ...
    >
      <el-form :model="subCategoryForm" :rules="subCategoryRules" ref="subCategoryFormRef" label-width="80px">
        ...
        <el-form-item label="排序" prop="sortOrder">
          <el-input-number
            v-model="subCategoryForm.sortOrder"
            :min="0"
            placeholder="排序值"
            style="width: 100%"
          />
...
const handleDeleteCategory = async (category) => {
  try {
    ...
    const response = await campaignCategoryApi.deleteCategory(category._id)
    if (response.success) {
...
const handleDeleteSubCategory = async (subCategory) => {
  try {
    ...
    const response = await campaignCategoryApi.deleteSubCategory(
  ...
  await categoryFormRef.value.validate(async (valid) => {
    if (valid) {
      try {
        ...
        let response
        if (categoryForm._id) {
          response = await campaignCategoryApi.updateCategory(categoryForm._id, categoryForm)
        } else {
          response = await campaignCategoryApi.createCategory(categoryForm)
        }
    ...
    if (valid) {
      try {
        ...
        if (subCategoryForm._id) {
          response = await campaignCategoryApi.updateSubCategory(
            selectedCategory.value._id, 
            subCategoryForm._id, 
            subCategoryForm
          )
        } else {
          response = await campaignCategoryApi.addSubCategory(selectedCategory.value._id, subCategoryForm)
        }
